...
import { resolveCompanyId } from "@/app/api/startup-connect/_shared";
...
export async function GET(req: Request) {
  try {
    ...
    if (gigReqs.length === 0) return NextResponse.json({ success: true, data: [] });

    const students = await prisma.users.findMany({
      where: { role: "STUDENT" },
      select: { id: true, name: true, specialization: true, skills: true, rating: true },
      take: 500,
    });
    ...
    const matches = students
      .map((student) => {
        ...
        for (const gigReq of gigReqs) {
          ...
          const matchPct = gigReq.length ? (matchedCount / gigReq.length) * 100 : 0;
      ...
      .sort((a, b) => (b.match !== a.match ? b.match - a.match : (parseFloat(b.rating) || 0) - (parseFloat(a.rating) || 0)))
